        ...
        if (!empty($person_wiki))
          {
            ...
            foreach ($init_summary as $part)
              {
                $person_summary .= strip_tags($part);
              }
            $person_summary = preg_replace("`&(amp;)?#?[a-z0-9]+;`i", "", $person_summary);
            $person_summary = preg_replace("/\.[0-9]+/", ".", $person_summary);
...
?>" class="img-fluid px-2 ratio ratio-9x16" onerror="this.onerror=''; this.src='img/no_image.png';">
                                    ...
                                    <div class="col-md-9">
                                        <h2 class="primary px-2"><b><?php echo $person_name; ?></b></h2>
                                        <h4 class="secondary px-2">
                                            <span
                                                class="badge rounded-pill bg-success"><?php echo $person_dob; ?></span>&nbsp;
                                            <span
                                                class="badge rounded-pill bg-warning"><?php echo $person_type; ?></span>&nbsp;
                                            <span class="badge rounded-pill bg-danger"><?php echo $person_sex; ?></span>
                                        </h4>
                                        <h5 class="secondary px-2" style="text-align:justify;">
                                            <?php
        echo $person_summary;
        unset($person_summary);
?>
                                        </h5>
                                    </div>
                                </div>
...
else
  {
    echo "<div class='alert alert-warning'>
              Ada kesalahan saat menampilkan halaman ini.
              </div>";
  }
?>
                </div>
                ...
                <div class="card-body m-2">
                    <div class="row">
                        <div class="col-md-12 mt-2">
                            <h4 class="primary pb-3 text-center">Memerankan / Menyutradarai</h4>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col my-4 d-flex justify-content-center">
                            <?php
$sql = "SELECT c.FILM_ID, f.FILM_TITLE, f.FILM_YEAR, f.FILM_WIKI FROM cast as c INNER JOIN film AS f ON f.FILM_ID = c.FILM_ID WHERE c.PERSON_ID = $id LIMIT 5;";
//execute query
$res = mysqli_query($conn, $sql);
//row presence check
$count = mysqli_num_rows($res);
            ...
            if (!empty($wiki_image))
              {
                ...
                if (strpos($wiki_image->src, "clapperboard") !== false || strpos($wiki_image->src, "microphone") !== false || strpos($wiki_image->src, "Soekarno") !== false || strpos($wiki_image->src, "Leistungen") !== false || strpos($wiki_image->src, "Flag_of_Indonesia") !== false || strpos($wiki_image->src, "bookmark_silver") !== false || strpos($wiki_image->src, "Semi-protection") !== false)
                  {
                    $film_image = " ";
                  }
              }
